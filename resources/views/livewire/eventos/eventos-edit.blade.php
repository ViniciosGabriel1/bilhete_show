<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg mt-5">
    ...
    <form wire:submit.prevent="atualizar" class="space-y-6">
        ...
        <!-- Nome do Evento -->
        <div>
            <label for="nome" class="block text-sm font-semibold text-gray-700">Nome do Evento</label>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label for="quantidade_disponivel" class="block text-sm font-semibold text-gray-700">Quantidade
                    Disponível</label>
        ...
        <div>
            ...
            <input type="file" id="imagem" wire:model="novaImagem"
                class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            @error('novaImagem')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        ...
        <!-- Exibir imagem antiga e nova lado a lado -->
        @if ($imagem || $novaImagem)
            <div class="flex m-5 gap-2 mt-4">
                
                <!-- Imagem Atual -->
                @if ($imagem)
                    <div class="flex flex-col items-center">
                        <p class="text-sm text-gray-500">Imagem Atual</p>
                        <img src="{{ asset('storage/' . $imagem) }}" class="rounded-lg shadow-md w-48 mt-2">
                    </div>
                @endif
                ...
                @if ($novaImagem)
                    <div class="flex flex-col items-center">
                        <p class="text-sm text-gray-500">Nova Imagem</p>
                        <img src="{{ $novaImagem->temporaryUrl() }}" class="rounded-lg shadow-md w-48 mt-2">
                    </div>
                @endif
